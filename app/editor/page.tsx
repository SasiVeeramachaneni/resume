...
export default function EditorPage() {
  usePageMeta(
    "Blog Editor | Create Resume",
    "Write and manage blog posts for Create Resume.",
    undefined,
    true,
  );
        ...
        <Paper withBorder p="lg" radius="md" mb="xl">
          ...
          <Group grow mb="sm">
            ...
            <TextInput
              ...
              placeholder="resume, career"
            />
          </Group>

          <Group mt="md">
            <Button onClick={handleSave}>
              {editing ? "Update Post" : "Save Post"} (localStorage)
            </Button>
            <Button variant="light" onClick={exportPost}>
              Export as Code
            </Button>
            {editing && (
          ...
          {exportCode && (
            <Paper withBorder p="sm" mt="md" bg="gray.0">
              <Text size="sm" fw={500} mb="xs">
                Copy this into app/blog/posts.ts:
              </Text>
          ...
          {lastSavedSlug && (
            <Alert
              ...
              mt="md"
              withCloseButton
              onClose={() => setLastSavedSlug(null)}
            >
              <Text size="sm">
                Add slug <Code>{lastSavedSlug}</Code> to{" "}
                <Code>app/blog/custom-posts.json</Code> for sitemap inclusion,
                then run <Code>npm run sitemap</Code>.
              </Text>
            </Alert>
          )}
        </Paper>
            ...
            <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
              {posts.map((post) => (
                <Card key={post.slug} withBorder padding="md" radius="md">
                  ...
                  <Group mt="sm" gap="xs">
                    <ActionIcon variant="light" onClick={() => editPost(post)}>
                      <IconEdit size={16} />
                    </ActionIcon>
                    <ActionIcon
                      variant="light"
